...
kp_theta = 4.0		#Work: 2.3 	  	#3.0	5.0		#6.0						#Angle Proportional Controller		3.5
ki_theta = 0.05		#Work: 0.005	#0.005	0.005	#0.05					  #Angle Integral Controller			0.05
kd_theta = 0.8	  	#Work: 1.0		#1.0	1.0		#3.0						#Angle Derivative Controller		1.0
...
DECELERATION = 7.0			#8.0 			#m/s  vehcile deceleration
MAX_ERRO_INTEGRADOR_DIS = 50.0	#UP limit for the integrator in distance
MAX_ERRO_INTEGRADOR_THETA = 1.0  #UP limit for the integrator in theta			#0.1
...
def direction_control (lat_TV, long_TV, h_TV, h_SV):
	...
	if (dt >= MINIMUM_DT):								  #act only after new controller variables
		...
		if (abs(shortest_angular_distance(heading_TV_OLD,heading_TV))<0.01):		#IF TV is not changing it's direction	
			...
			#print "data ESSE: ", latitude_SV, longitude_SV, heading_SV, latitude_TV, longitude_TV
			...
			#print "data AQUELE: ", latitude_SV, longitude_SV, heading_SV, lat_TV, long_TV
		...
		#if (abs(theta_error) < BEARING_ANGLE_LIM  and BEARING_CONTROL):
...
def vehicle_general_control():
	...
	if ((throttle_TV == -100.0 or brake_TV == 1.0) and FULL_BRAKE) :				#perform a FULLBRAKE when TV stops (CASE 1)
		...
		if (distance_error < -4.0):
			...
			msg.throttle, msg.brake = 0.0 , 0.95											#determine throttle and brake
		msg.steer = steering_SV														#keeps the steering from previous iteractions to avoid errors
		if 1:
			print "-----------------"
			print "TV Brake 2"
	...
	if (abs(msg.steer) >= (MAX_STEER * ANG_TO_RAD)):
		msg.throttle = msg.throttle/3.0 
		msg.brake = msg.brake *3.0		#determine throttle and brake
