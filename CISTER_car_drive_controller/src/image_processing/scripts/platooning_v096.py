...
TV_position_vector = np.empty([1,5])					#vector to store TV positions to be followed
...
def direction_control (lat_TV, long_TV, h_TV, h_SV):
	...
	if (dt >= MINIMUM_DT):								  #act only after new controller variables
		...
		#if (abs(theta_error) < BEARING_ANGLE_LIM and dist > BEARING_DIST_LIM and BEARING_CONTROL and (abs(d_lat)>0.2 and abs(d_long)>0.2)):
...
def compare_lost_position():
	...
	if mode== LOST:
		...
		print "----------------->LOST: ",count
		print "TV_position_vector: ", TV_position_vector[pos_lost,1], TV_position_vector[pos_lost,0] , TV_position_vector[pos_lost,2]
		print "SV:", longitude_SV, latitude_SV, heading_SV, speed_SV
		print "Heading_SV_adjusted: ",head_SV_adjusted
		print "thetas: ", theta_SV_orig_salvo, theta_teste, diff_teste
		...
		#	print TV_position_vector[k,0], TV_position_vector[k,1], TV_position_vector[k,2] , TV_position_vector[k,3] , TV_position_vector[k,4]
		count = count + 1
	else:
		speed_out = TV_position_vector[pos,3]
...
def vehicle_general_control():
	...
	if (TV_position_vector[0,0]<=0.1 and TV_position_vector[0,1]<=0.1):												 #cleaning the first position of the vector
		TV_position_vector = np.append(TV_position_vector,[[latitude_TV, longitude_TV, heading_TV, speed_TV, tp.time()]],axis=0)   #Numpy vector que armazena posicoes de TV
