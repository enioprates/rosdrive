...
longitude_TV_old = 0.0									   #longitude
latitude_TV_old = 0.0										#latitude
speed_TV_OLD = 0.0 
...
kp_theta = 4.2		#Work: 4.0 	  							#Angle Proportional Controller		3.5			4.0
ki_theta = 0.01		#Work: 0.005						  #Angle Integral Controller			0.05		0.05
kd_theta = 1.0	  	#Work: 0.8								#Angle Derivative Controller		1.0			0.8
...
TV_PID_error = 0.0										#Distance error from TV
TV_THETA_error = 0.0									  #Theta error from TV
TV_PID_error_OLD = 0.0
TV_THETA_error_OLD = 0.0
...
ANG_TO_RAD	= math.pi/180			#converte graus para radiano
RAD_TO_ANGLE = 180/math.pi		#converte radiano para graus
...
DECELERATION = 6.0			#8.0 			#m/s  vehcile deceleration
MAX_ERRO_INTEGRADOR_DIS = 50.0	#UP limit for the integrator in distance
MAX_ERRO_INTEGRADOR_THETA = 0.5  #UP limit for the integrator in theta			#0.1	1.0
BEARING_ANGLE_LIM = 0.15		#MIN limit for bearing check (angle)			#0.15
#BEARING_ANGLE_LIM = 0.015		#MIN limit for bearing check (angle)
#BEARING_ANGLE_LIM = 0.2618		#MIN limit for bearing check (angle)
BEARING_DIST_LIM = 1.0		#MIN limit for bearing check (DIST)					#1.0

BEARING_CONTROL = ON			#activate or desactivate the BEARING conbtrol
INITIAL_SPEED = ON			  	#define a initial speed
flag_initial_speed = ON
FULL_BRAKE = ON					#enable the full brake when the TV Stops

#---------just one lap
TARGET_SPEED = 15.00			#target speed (approx 50KM/h) ->just to set the initial system's speed
...
def calc_throttle_brake(desired_speed, real_speed):
	...
	global flag_initial_speed

	if flag_initial_speed:
		desired_speed = TARGET_SPEED

	if real_speed >= TARGET_SPEED:
		flag_initial_speed = OFF
...
def bearing_calculator_v2(d_long, dist):
	...
	bearing_angle = normalize_angle (bearing_angle)
...
#-----------------------------------------------------------------------
#FUNCAO: bearing_calculator_v3(d_lat, d_long, theta_SV)
#	calculates the bearing between SV and the desired position from TV
#   return bearing angle
#	
#	input:	d_lat 		-> 	lat difference
#			d_long 		->	long difference
#			theta_SV	->	SV angle
#-----------------------------------------------------------------------
def bearing_calculator_v3(d_lat, d_long, theta_SV):

	global new_message
	global bearing_angle_old
	...
	if (new_message):
		if (d_lat > 0.0):
			#print "d_lat > 0.0"
			alpha = math.atan(d_long/d_lat)
			#bearing_angle = - (theta_SV - alpha)					 #bearing
			bearing_angle = - shortest_angular_distance(alpha,theta_SV)					 #bearing
		elif (d_lat < 0.0 and abs(d_long) > 0.0):
			#print "d_lat < 0.0 and abs(d_long) > 0.0"

			alpha = math.atan(d_long/d_lat)
			bearing_angle = shortest_angular_distance(theta_SV,math.pi) + alpha					 #bearing
			#print "shortest_angular_distance(math.pi,theta_SV): ", shortest_angular_distance(theta_SV,math.pi)
		else:
			bearing_angle = 0.0								 #bearing
		bearing_angle_old = bearing_angle
	else:
		bearing_angle = bearing_angle_old
		alpha = 0.0

	if debug:
		print "bearing calculator"
		print "d_lat, d_long, theta_SV: ", d_lat, d_long, theta_SV
		print "alpha: ",alpha, alpha *RAD_TO_ANGLE
		print "bearing_angle: ", bearing_angle, bearing_angle * RAD_TO_ANGLE
	return bearing_angle
...
def direction_control (lat_TV, long_TV, h_TV, h_SV):
	...
	if (dt >= MINIMUM_DT):								  #act only after new controller variables
		...
		if (abs(shortest_angular_distance(heading_TV_OLD,heading_TV))<0.01):		#IF TV is not changing it's direction	
			...
			#print "data ESSE: ", latitude_SV, longitude_SV, heading_SV, latitude_TV, longitude_TV, d_lat
			...
			#print "data AQUELE: ", latitude_SV, longitude_SV, heading_SV, lat_TV, long_TV, d_lat
		...
		#if (abs(theta_error) < BEARING_ANGLE_LIM and dist > BEARING_DIST_LIM  and BEARING_CONTROL and new_message):
		if (abs(theta_error) < BEARING_ANGLE_LIM and dist > BEARING_DIST_LIM  and BEARING_CONTROL):
			...
			bearing_angle = bearing_calculator_v3(d_lat, d_long, theta_SV)  #calls the bearing angle function
			if debug:
				print "d_lat, d_long, theta_SV: ", d_lat, d_long, theta_SV
			...
			if debug:
				 ...
				 print "Theta_error + Bearing: ", theta_error
			...
			theta_error = normalize_angle(theta_error)
			#print "Theta_error + Bearing (normalized): ", theta_error
		else:
			bearing_angle_old = ZERO
		...
		if (abs(theta_error) >= EDGE_ANGLE):							#If theta_error is significative
			...
			theta_deriv = shortest_angular_distance(theta_error,theta_erro_old)
			...
			#theta_control = normalize_angle (theta_control)
			...
			if (theta_control > MAX_STEER * ANG_TO_RAD):		#adjust the maximum control action
				if debug:
					...
					print "-----------"
				theta_control = MAX_STEER * ANG_TO_RAD #- random.uniform(0.0,0.05)
...
def compare_lost_position():
	...
	for i in range(TV_position_vector.shape[0]):			#for each position in TV_position_vector
		...
		if (frente):
			...
			if dist_min >= dist and dist > 0.0:
				...
				print "i: ", i
				print "shape[i]: ", TV_position_vector.shape[0]
				pos = i
				mode = LOST
				print "aqui 3 - LOST"
		else:
			if 1:
				"VALOR ATRAS - DESCARTAR"
	...
	if debug:
		...
		for k in range(TV_position_vector.shape[0]):
			...
			count = count + 1
		print "SV:", longitude_SV, latitude_SV, heading_SV, speed_SV
		print "TV_position_vector: ", TV_position_vector[pos,1], TV_position_vector[pos,0], TV_position_vector[pos,2], TV_position_vector[pos,3], TV_position_vector[pos,5]
...
def vehicle_general_control():
	...
	global latitude_TV_old
	global longitude_TV_old
	global speed_TV_OLD
	...
	global distance_old
	global new_message
	global TV_PID_error_OLD
	global TV_THETA_error_OLD
	...
	if (latitude_TV <> latitude_TV_old or longitude_TV<>longitude_TV_old or speed_TV<>speed_TV_OLD ):
		longitude_TV_old = longitude_TV
		latitude_TV_old = latitude_TV
		speed_TV_OLD = speed_TV
		distance = ahead_position(latitude_SV,longitude_SV,heading_SV,latitude_TV,longitude_TV)[2]
		distance_old = distance
		TV_PID_error_OLD = TV_PID_error
		TV_THETA_error_OLD = TV_THETA_error
		new_message = True
		#print "-------------NEW MESSAGE------------------"
	else:
		distance = distance_old
		TV_PID_error = TV_PID_error_OLD
		TV_THETA_error = TV_THETA_error_OLD
		new_message = False

	if abs(speed_TV - speed_TV_reference) > 0.2:																		#adjust the speed only when the TV changes more then 1 m/s
	...
	maximum_distance = MINIMUM_DISTANCE + speed_SV * SAFETY_TIME - (DECELERATION * SAFETY_TIME * SAFETY_TIME)/2
	#maximum_distance = 13.0
	...
	if debug:
		...
		print "Lat_Tv, Long_TV: ", latitude_TV,longitude_TV
		print "Lat_Sv, Long_SV: ", latitude_SV,longitude_SV
	...
	if ((throttle_TV == -100.0 or brake_TV == 1.0) and FULL_BRAKE) :				#perform a FULLBRAKE when TV stops (CASE 1)
		...
		#print "heading_compare: ", heading_compare, " TV_head_acum: ", TV_head_acum, " heading_TV: ", heading_TV
		speed_compare = speed_TV

		if incremental_controller:
			#PID_dist = distance_control (distance_error + TV_PID_error)				#determines de control action for the distance
			PID_dist = distance_control (distance_error + TV_dist_acum)				#determines de control action for the distance
		else:
		...
		# if new_speed_SV > speed_compare + 1.0 and speed_compare > 1.0:
		# 	new_speed_SV = speed_compare + 1.0
		...
		#if (msg.steer >= 0.15):
		#	msg.throttle, msg.brake = calc_throttle_brake(speed_compare,speed_SV)		#determine throttle and brake
	...
	if debug:
		print "MSG.STEERING: ", msg.steer

	#if (abs(msg.steer) >= (MAX_STEER * ANG_TO_RAD)):
	#	msg.throttle = msg.throttle/1.1 
	#	msg.brake = msg.brake *2.0		#determine throttle and brake
...
def SV_info(data):
	...
	if (latitude_TV <> 0.0 and longitude_TV <> 0.0):
...
if __name__ == '__main__':
	...
	# # print "frente/tras: ", ahead_position(th_sv_lat, th_sv_lon, th_sv, th_tv_lat, th_tv_lon)[0]
	# # print "dist_y: ", ahead_position(th_sv_lat, th_sv_lon, th_sv, th_tv_lat, th_tv_lon)[1]
	# # print "dist_x: ", ahead_position(th_sv_lat, th_sv_lon, th_sv, th_tv_lat, th_tv_lon)[2]

	# d_lat = th_tv_lat - th_sv_lat
	# d_long = th_tv_lon - th_sv_lon

	# bearing_calculator_v3(d_lat, d_long, th_sv)
